"""Plot led_fluctuation_experiment.csv (columns: t_s, adc, volts, led)."""
...
CSV = "led_fluctuation_experiment.csv"
...
ax_adc.axhline(mean_adc_on, color="C3", linestyle="--", linewidth=0.8,
               label=f"LED-ON mean = {mean_adc_on:.1f}")
ax_adc.fill_between(
    t, mean_adc_on - std_adc_on, mean_adc_on + std_adc_on,
    color="C3", alpha=0.10,
    label=f"±1σ ({std_adc_on:.1f} counts, {cov_on_pct:.1f}% CoV)"
)
...
ax_v.axhline(v_on.mean(), color="C3", linestyle="--", linewidth=0.8,
             label=f"LED-ON mean = {v_on.mean():.4f} V")
ax_v.set_xlabel("time (s)")
ax_v.set_ylabel("voltage (V)")
ax_v.legend(loc="lower right", framealpha=0.9)
ax_v.grid(True, alpha=0.3)

# Shade LED-OFF windows.
off = led == 0
if off.any():
...
fig.suptitle(
    "LED fluctuation experiment (ItsyBitsy M0, TCC0 direct NPWM 200 Hz, 12-bit ADC)"
)
fig.tight_layout()
fig.savefig("led_fluctuation_experiment.png", dpi=150)

# Also emit a histogram of LED-ON counts to make any regime split obvious.
fig2, ax_h = plt.subplots(figsize=(8, 4))
bins = max(20, int(adc_on.max() - adc_on.min()) // 5)
...
ax_h.set_title(f"LED-ON ADC distribution (n = {on.sum()}, "
               f"range {adc_on.min():.0f}..{adc_on.max():.0f})")
ax_h.grid(True, alpha=0.3, axis="y")
fig2.tight_layout()
fig2.savefig("led_fluctuation_hist.png", dpi=150)

print(f"n total = {len(t)}, LED-on samples = {on.sum()}, LED-off = {(~on).sum()}")
print(f"LED-ON ADC: mean = {mean_adc_on:.2f}, std = {std_adc_on:.2f}, "
      f"CoV = {cov_on_pct:.2f}%, range = {adc_on.min():.0f}..{adc_on.max():.0f}")
print("wrote led_fluctuation_experiment.png and led_fluctuation_hist.png")
