// Absorbance measurement firmware, Pro Micro nRF52840 (Rook-compatible), 6.0PMPM.
//
...
// Pro Micro nRF52840 pin map (Arduino pin numbers per variant.h):
//   A -> Arduino 6  (P1.00)  matches Rook PIN_USER_BTN
//   B -> Arduino 9  (P1.06)  free GPIO on Rook by default
//   Detect -> Arduino 15 (P0.02, AIN0)  first ADC-capable pin
//   I2C SDA/SCL are fixed by variant.h to Arduino 8 / 7.
#define BTN_A_PIN     6   // A -> restart / abort
...
static void render() {
  ...
  // Version tag, top-right corner ("6.0PM" = 5 chars * 6 px = 30 px).
  display.setCursor(OLED_W - 5 * 6, 0);
  display.print("6.0PM");
...
void setup() {
  ...
  Serial.println("# 6.0PM (nRF52840) ready");
