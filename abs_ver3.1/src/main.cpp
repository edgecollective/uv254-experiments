// Absorbance measurement firmware, ItsyBitsy nRF52840, ver3.1 variant.
//
...
static void render() {
  ...
  // Version tag, top-right corner ("ver3.1" = 6 chars * 6 px = 36 px wide).
  display.setCursor(OLED_W - 6 * 6, 0);
  display.print("ver3.1");
...
static void on_calibrate_press() {
  if (state != STATE_READY) return;   // silently ignored during warm-up
...
void setup() {
  ...
  Serial.println("# ver3.1 (nRF52840) ready");
