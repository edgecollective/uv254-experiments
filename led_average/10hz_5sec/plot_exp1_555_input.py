"""Plot led_average/10hz_5sec/exp1_555_input.csv (columns: t_s, adc, volts, led).

Same firmware and averaging (10 Hz sample, 50-sample avg -> 5 s cadence) as
exp1.py, but the LED is being driven by the on-board 555 timer (~983 Hz,
~5 V swing) instead of the micro's TCC0 200 Hz output.
"""
...
CSV      = "exp1_555_input.csv"
OUT_TS   = "exp1_555_input.png"
OUT_HIST = "exp1_555_input_hist.png"
SUPTITLE = ("led_average 10Hz/5s exp1_555_input (ItsyBitsy M0, 555-driven "
            "LED ~983 Hz, 12-bit ADC, 50-sample avg -> row every 5 s)")

# Same y-axis bounds as exp1.py so the two runs are directly comparable.
ADC_YMAX = 800
V_YMAX   = 0.62
