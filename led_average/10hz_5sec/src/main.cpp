...
// Averaging schedule: 50 samples at 10 Hz = one 50-point average every 5 s.
const uint32_t SUBSAMPLE_INTERVAL_MS = 100;   // 10 Hz
const uint16_t SAMPLES_PER_AVERAGE   = 50;    // 5 s window
...
static void render() {
  ...
  display.println("led_average 10Hz/5s");
