// Basic LOD test with cuvette swaps, ItsyBitsy nRF52840.
//
// Boot sequence:
//   1) 2-min warm-up (buttons disabled).
//   2) Firmware prompts the operator to insert / swap the blank cuvette.
//   3) Operator presses B when the cuvette is seated.
//   4) One measurement: 5 rounds settle (25 s) + 10 rounds avg (50 s).
//   5) Firmware records the final and prompts for the next swap.
//   6) Repeat up to MAX_BLANKS times, or until A (stop) is pressed.
//
// Each round = 50 samples @ 10 Hz = 5 s average. Final I_blank for each
// measurement is the mean of the 10 avg rounds.
//
// Buttons:
//   B (pin 9) -> ADVANCE (start next blank; ignored during warm-up / measuring)
//   A (pin 7) -> STOP    (ends the run; ignored during warm-up)
//
...
#define BTN_ADV_PIN   9   // Button B -> ADVANCE
#define BTN_STOP_PIN  7   // Button A -> STOP
...
enum State    { STATE_WARMUP, STATE_PROMPT_SWAP, STATE_MEASURING, STATE_STOPPED };
...
uint16_t blank_count  = 0;   // number of completed blanks
...
Button btn_adv  = {BTN_ADV_PIN,  HIGH, 0};
Button btn_stop = {BTN_STOP_PIN, HIGH, 0};
...
static void render() {
  ...
  if (state == STATE_WARMUP) {
    ...
    display.println("basic_LOD_swap");
  } else if (state == STATE_PROMPT_SWAP) {
    uint16_t next = blank_count + 1;
    if (next == 1) {
      display.println("Insert blank cuvette");
    } else {
      display.print("SWAP cuvette for");
      display.setCursor(0, 8);
      display.print("blank #");
      display.println(next);
    }
    display.setCursor(0, 20);
    display.print("Then press B (");
    display.print(blank_count);
    display.print('/');
    display.print(MAX_BLANKS);
    display.println(")");
...
static void enter_prompt_swap() {
  state = STATE_PROMPT_SWAP;
  subphase = SUB_NONE;
  uint16_t next = blank_count + 1;
  if (next > MAX_BLANKS) {
  ...
  Serial.print("# awaiting cuvette swap for blank ");
  Serial.print(next);
  Serial.print(" (press B when seated) at t=");
...
static void finish_warmup() {
  ...
  Serial.print((millis() - boot_ms) / 1000.0f, 3);
  Serial.println("s; ready for blank cycles");
  Serial.flush();
  enter_prompt_swap();
}

static void on_advance_press() {
  if (state != STATE_PROMPT_SWAP) return;   // ignored otherwise
  start_next_blank();
}
...
static void tick_sampling() {
  // Sample during warm-up (for the log) and during measuring; not during
  // prompt-swap or stopped.
...
void setup() {
  ...
  pinMode(BTN_ADV_PIN,  INPUT_PULLUP);
  ...
  Serial.println("# basic_LOD_swap (nRF52840) ready");
  ...
  Serial.print  (MAX_BLANKS);
  Serial.println(" blanks with manual cuvette swaps");
  Serial.println("# B=ADVANCE (start next blank), A=STOP");
...
void loop() {
  check_button(btn_adv,  on_advance_press);
  check_button(btn_stop, on_stop_press);
  tick_sampling();
}
