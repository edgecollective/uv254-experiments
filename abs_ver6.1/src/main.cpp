// Absorbance measurement firmware, ItsyBitsy nRF52840, ver6.1 variant.
//
// ver6.1 changes vs ver6.1: replaces the single ambiguous "Absorb=" screen
// line with two labeled rows so raw and corrected absorbance are both visible
// at a glance:
//     A_raw= <n>  k=<n>
//     A_corr=<n>
// If STEP 2/3 (standard) has not run yet, k=1.000 and A_corr equals A_raw --
// which is exactly the visual cue that no slope correction is being applied.
//
...
static void render() {
  ...
  // Top-right tag. When the standard has been measured (2-pt cal complete),
  // prefix the version with "CAL " so it is unmistakable at a glance that
  // the slope correction is being applied.
  if (have_std) {
    display.setCursor(OLED_W - 10 * 6, 0);   // "CAL ver6.1" = 10 chars
    display.print("CAL ver6.1");
  } else {
    display.setCursor(OLED_W - 6 * 6, 0);    // "ver6.1"     = 6 chars
    display.print("ver6.1");
  }
  ...
  // Four rows at 8 px spacing so both raw and corrected absorbance are
  // simultaneously visible. When have_std is false, k = 1.000 and A_corr
  // equals A_raw -- the very fact that the two numbers are identical is the
  // visual cue that no slope correction is being applied.
...
void setup() {
  ...
  Serial.println("# ver6.1 (nRF52840) ready");
